...
def describe_time_delta(
    ...
    if days:
        num_days, remaining_seconds = divmod(remaining_seconds, _SECONDS_IN_DAY)
        # round up if there is a lower granularity component we're not displaying
        num_days += 1 if remaining_seconds and not (hours or minutes or seconds) else 0
        if num_days:
            description_components.append("{} day(s)".format(num_days))

    if hours:
        num_hours, remaining_seconds = divmod(remaining_seconds, _SECONDS_IN_HOUR)
        # round up if there is a lower granularity component we're not displaying
    ...
    if minutes:
        num_minutes, remaining_seconds = divmod(remaining_seconds, _SECONDS_IN_MINUTE)
        # round up if there is a lower granularity component we're not displaying
