...
def extract_win_streaks_for_characters(characters: dict):
    characters = itertools.chain.from_iterable(
        x.values() for x in characters.values())
    ...
    def map_characters_to_win_streaks(character: dict) -> Tuple[int, int]:
        ...
        if sorted_seasons:
            ...
            race_win_streaks = (x.get("current_win_streak", 0)
                                ...
                                time.time() - _SECONDS_IN_5_DAYS)

            race_longest_win_streaks = (x.get(
                "longest_win_streak",
...
class FetchMemberIdsDatabaseTask(DatabaseTask[List[str]]):
    def execute_with_database(self,
                              db: pyrebase.pyrebase.Database) -> List[str]:
...
class FetchAllinMembersDatabaseTask(DatabaseTask[dict]):
    def execute_with_database(self, db: pyrebase.pyrebase.Database) -> dict:
        members = db.child("members").order_by_child(
            "is_full_member").equal_to(True).get().val()
        return members if members else {}
...
class WinStreakHandler(Handler):
    ...
    def can_handle_message(self, message: discord.Message) -> bool:
        return message.content in [_TRIGGER, _ALT_TRIGGER
