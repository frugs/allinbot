...
_LEAGUE_EMBLEMS = [
    "<:bronze:230151914812211210>", "<:silver:230151947498291211>",
    "<:gold:230151929236291585>", "<:platinum:230151940275830795>",
    "<:diamond:230151923276185611>", "<:masters:230151935053922305>",
...
class Sc2LadderInfoHandler(Handler):
    ...
    async def handle_message(self, client: discord.Client,
        ...
        ladder_info = await perform_database_task(
            client.loop,
            RetrieveLadderInfoDatabaseTask(discord_id, self._db_config))

        if not ladder_info:
            message_to_send = "Could not find ladder info for <@{}>.".format(
                discord_id)
        else:
            ...
            races_played = [
                race for race in _RACES
                if ladder_info.get(race.lower() + "_player")
            ]
            if races_played:
                message_to_send += "Highest ranked race(s): " + ", ".join(
                    races_played) + "\n"
